...
local profile = {
	...
	preference = {
		...
		---Enable LSP inlay hint
		---@type boolean
		enable_inlay_hint = vim.lsp.inlay_hint.is_enabled(),
		...
		---Each will be merged with `opts' of corresponding plugin
		---@type table<string, table | fun(self: LazyPluginSpec, opts: table, extras: table?): table?>
		plugin_opts = {
			-- e.g.
			-- codecompanion = {
			-- 	strategies = {
	...
	---Appearence
	---@class profiles.Profile.Appearance
	appearence = {
		---@class profiles.Profile.Appearance.Theme
		...
		theme = {
			...
			---Which theme plugin to use
			---Can be a spec module name from "plugins/themes/"
			---Or can be a `LazyPluginSpec` that will be sent to lazy.nvim
